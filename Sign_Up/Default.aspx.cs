...
public partial class Sign_Up_Default : System.Web.UI.Page
{
    ...
    protected void Sign_Up_Submit_Click(object sender, EventArgs e)
    {
        ...
        if (!isValidForm)
        {
            Error_Label.Text = "Invalid Student Name.";
            return;
        }

        //BITS ID
        isValidForm = MyValidator.Required(BITS_ID_TextBox.Text) && MyValidator.noSpecialChars(BITS_ID_TextBox.Text) && MyValidator.exactly(BITS_ID_TextBox.Text, 12);
        if (!isValidForm || CheckBITSID(BITS_ID_TextBox.Text) == 0)
        {
            Error_Label.Text = "Invalid BITS ID or BITS ID already registerted";
            return;
        }
        ...
        //Hostel
        isValidForm = MyValidator.ValidateListView(Hostel_DropDownList.SelectedIndex);
        if (!isValidForm)
        {
            Error_Label.Text = "Select your hostel";
            return;
        }

        //Room
        isValidForm = MyValidator.Required(Room_No_TextBox.Text) && MyValidator.exactly(Room_No_TextBox.Text, 3) && MyValidator.onlyNumber(Room_No_TextBox.Text);
        if (!isValidForm)
        {
            Error_Label.Text = "Invalid Room no.";
            return;
        }
        ...
        using (SqlConnection con = ConnectionManager.Connect())
        {
            try
            {
                SqlCommand mySql = new SqlCommand("insert into EBay_TempStudent (Username_vc, Password_vc, Student_Name_vc, IsAdmin_bool, Hostel_nc, BITS_ID_vc, Room_No_nc, Phone_No_vc, SecurityKey_nc, PhotoURL, ItemsCloud_vc, StudentsCloud_vc) VALUES ('"
                    ...
                    + Hostel_DropDownList.SelectedValue + "', '" + BITS_ID_TextBox.Text + "', '" + Room_No_TextBox.Text + "', '"
